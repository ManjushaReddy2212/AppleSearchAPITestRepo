...
public class AppleSearchAPIAutomation {
    ...
    public void searchByTermAndMediaType(String term, String mediaType) {
        ...
        assertTrue(resultCount < 51, "Default result count should not be more than 50. But the actual count is " + resultCount);
        ...
        for (int i = 0; i < resultCount; i++) {
            ...
            // For audio-book it needs special handling as kind key is absent, rather entity info is present in wrapperType key           
            ...
            Assert.assertTrue(supportedEntities.contains(entityType), "Invalid entity type is returned " + "media: " + mediaType
    ...
    private HashSet<String> getValidEntityTypesForMedia(String mediaType) {
        ...
        // Table: https://developer.apple.com/library/archive/documentation/AudioVideo/Conceptual/iTuneSearchAPI/Searching.html#//apple_ref/doc/uid/TP40017632-CH5-SW2
        ...
        case "ebook":
            ...
            break;
        case "all":
            entities = new HashSet<>(Arrays.asList("movie", "album", "allArtist", "podcast", "musicVideo", "mix", "audiobook", "tvSeason", "allTrack"));
            break;
        default:
    ...
    public void testStatusCodesForApI() {
        ...
        response = given().log().all().queryParam("term", "test").queryParam("media", "InvalidMediaType").get(Constants.API);
        statusCode = response.getStatusCode();
        ...
        response = given().log().all().queryParam("term", "test").queryParam("country", "InvalidCountryType").get(Constants.API);
        statusCode = response.getStatusCode();
        Assert.assertEquals(statusCode, Constants.HTTP_INVALID_INPUT, "Expected status code is 400");
    ...
    public void testPricePointsAndCountryForAPI(String country, String currency, String countryCode) {
        ...
        String response = given().log().all().queryParam("term", "Jim jones").queryParam("country", country ).queryParam("limit", 1).when().get(Constants.API).then()
                .statusCode(Constants.HTTP_OK_STATUS).extract().response().asString();
    ...
    public void validateLimit(String term, String mediaType) {
      ...
      String response = given().log().all().queryParam("term", term).queryParam("media", mediaType).queryParam("limit", 1).when().get(Constants.API).then()
              .statusCode(Constants.HTTP_OK_STATUS).extract().response().asString();
      ...
      String response1 = given().log().all().queryParam("term", term).queryParam("media", mediaType).queryParam("limit", 4).when().get(Constants.API).then()
              .statusCode(Constants.HTTP_OK_STATUS).extract().response().asString();
    ...
    //None of the API response fields match to Attribute type. So this method cannot be used for validations
    private HashSet<String> getValidAttributeTypesForMedia(String mediaType) {
        
        // Table: https://developer.apple.com/library/archive/documentation/AudioVideo/Conceptual/iTuneSearchAPI/Searching.html#//apple_ref/doc/uid/TP40017632-CH5-SW3
        HashSet<String> attributes;
        switch (mediaType) {

        case "movie":
            attributes = new HashSet<>(Arrays.asList("actorTerm", "genreIndex", "artistTerm", "shortFilmTerm", "producerTerm", "ratingTerm", "directorTerm", "releaseYearTerm", "featureFilmTerm", "movieArtistTerm", "movieTerm", "ratingIndex", "descriptionTerm"));
            break;
        case "podcast":
            attributes = new HashSet<>(Arrays.asList("titleTerm", "languageTerm", "authorTerm", "genreIndex", "artistTerm", "ratingIndex", "keywordsTerm", "descriptionTerm"));
            break;
        case "music":
            attributes = new HashSet<>(Arrays.asList("mixTerm", "genreIndex", "artistTerm", "composerTerm", "albumTerm", "ratingIndex", "songTerm"));
            break;
        case "musicVideo":
            attributes = new HashSet<>(Arrays.asList("genreIndex", "artistTerm", "albumTerm", "ratingIndex", "songTerm"));
            break;
        case "audiobook":
            attributes = new HashSet<>(Arrays.asList("titleTerm", "authorTerm", "genreIndex", "ratingIndex"));
            break;
        case "shortFilm":
            attributes = new HashSet<>(Arrays.asList("genreIndex", "artistTerm", "shortFilmTerm", "ratingIndex", "descriptionTerm"));
            break;
        case "tvShow":
            attributes = new HashSet<>(Arrays.asList("genreIndex", "tvEpisodeTerm", "showTerm", "tvSeasonTerm", "ratingIndex", "descriptionTerm"));
            break;
        case "software":
            attributes = new HashSet<>(Arrays.asList("softwareDeveloper"));
            break;
        case "all":
            attributes = new HashSet<>(Arrays.asList("actorTerm", "languageTerm", "allArtistTerm", "tvEpisodeTerm", "shortFilmTerm", "directorTerm", "releaseYearTerm", "titleTerm", "featureFilmTerm", "ratingIndex", "keywordsTerm", "descriptionTerm", "authorTerm", "genreIndex", "mixTerm", "allTrackTerm", "artistTerm", "composerTerm", "tvSeasonTerm", "producerTerm", "ratingTerm", "songTerm", "movieArtistTerm", "showTerm", "movieTerm", "albumTerm"));
            break;
        default:
            throw new IllegalArgumentException("Invalid media type");
        }
    ...
    //API response includes both explicit and not explicit results irrespective of explicit flag       
    ...
    ////API response is always in English irrespective of language field, zh_CN,ja_JP
